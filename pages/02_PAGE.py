...
def create_prompt(selected_grade, task_input):
    prompt_template = f"""
    {selected_grade}가 "초등학교 3~4학년"이면 "[]"안에 숫자가 "4"로 시작하는 학습 성취기준 코드를 찾고, {selected_grade}가 "초등학교 5~6학년"이면 학습 성취기준은 "[]"안에 숫자가 "6"으로 시작하는 학습 성취기준 코드를 찾고, {selected_grade}가 "중학교 1~3학년"의 학습 성취기준은 "[]"안에 숫자가 9로 시작하는 학습 성취기준 코드를 찾습니다. 
    "초등학교 3~4학년군"의 학습 성취기준은 다음과 같습니다:
    ...
    이러한 내용을 참고하여 성취기준을 찾는 방법을 알고, {selected_grade}의 학습 성취기준 중에 {task_input}와 관련된 성취기준을 수정하지 말고, 그대로 모두 찾아서 성취기준 코드와 성취기준 내용을 표로 만들어주세요.
