...
def create_prompt(selected_grade, task_input):
    prompt_template = f"""
    ### 지시사항:
    1. {selected_grade}가 "초등학교 3~4학년"이면, 성취기준 코드가 숫자 "4"로 시작하는 항목만 찾으세요.
    2. {selected_grade}가 "초등학교 5~6학년"이면, 성취기준 코드가 숫자 "6"으로 시작하는 항목만 찾으세요.
    3. {selected_grade}가 "중학교 1~3학년"이면, 성취기준 코드가 숫자 "9"로 시작하는 항목만 찾으세요.
    4. {task_input}과 관련된 단원의 성취기준 코드와 내용을 모두 찾은 다음 그 중에서 {selected_grade}에 맞는 학습 성취기준 코드에 해당하는 성취기준을 수정하지 말고, 성취기준 코드와 성취기준 내용을 표로 만들어주세요.
    
    ### 예시:
    "초등학교 3~4학년"에서 "힘과 우리 생활" 단원의 성취기준 코드와 내용은 다음과 같습니다:
